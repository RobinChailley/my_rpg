...
void 				camera_move_up_1(all_t *all, object_t *tmp,
...
void 				camera_move_up_2(all_t *all, button_t *btn,
text_t *txt)
{
	while (btn) {
		if (BTN_MOVABLE_CAM) {
...
void 				camera_move_up(all_t *all)
{
	...
	if (all->player.walking == 1) {
		camera_move_up_1(all, tmp, tmp_s, mob);
		camera_move_up_2(all, btn, txt);
	}
}