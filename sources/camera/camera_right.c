...
void 				camera_move_right_1(all_t *all, object_t *tmp,
shot_t *tmp_s, mob_t *mob)
{
	...
	while (tmp_s != NULL) {
		tmp_s->pos.x += all->player.speed_w;
		sfSprite_setPosition(tmp_s->sprite, tmp_s->pos);
		tmp_s = tmp_s->next;
	}
	while (mob) {
		mob->pos.x += all->player.speed_w;
		sfSprite_setPosition(mob->sprite, mob->pos);
		mob = mob->next;
	}
}

void 				camera_move_right_2(all_t *all, button_t *btn,
text_t *txt)
{
	while (btn) {
		if (BTN_MOVABLE_CAM) {
			btn->pos.x += all->player.speed_w;
...
void 				camera_move_right(all_t *all)
{
	...
	if (all->player.walking == 1) {
		camera_move_right_1(all, tmp, tmp_s, mob);
		camera_move_right_2(all, btn, txt);
	}
}